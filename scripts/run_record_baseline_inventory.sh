...
run_id="caco_trackA_baseline_inventory_$(date -u +%Y%m%dT%H%M%SZ)"
run_dir="progress/runs/$run_id"
mkdir -p "$run_dir"
printf '%s\n' "python -u caco_trackA_v1/scripts/record_baseline_inventory.py --output $run_dir/inventory.json" > "$run_dir/command.txt"
...
python -u caco_trackA_v1/scripts/record_baseline_inventory.py --output "$run_dir/inventory.json" \
