...
run_id="caco_trackA_feature_integrity_$(date -u +%Y%m%dT%H%M%SZ)"
run_dir="progress/runs/$run_id"
mkdir -p "$run_dir"
printf '%s\n' "python -u caco_trackA_v1/scripts/validate_features.py" > "$run_dir/command.txt"
...
python -u caco_trackA_v1/scripts/validate_features.py \
