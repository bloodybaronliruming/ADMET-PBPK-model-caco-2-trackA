...
run_id="caco_trackA_baseline_summary_$(date -u +%Y%m%dT%H%M%SZ)"
run_dir="progress/runs/$run_id"
mkdir -p "$run_dir"
if [[ "${1:-}" == "--check-readiness" ]]; then
    arguments=(--check-readiness --readiness-report "$run_dir/readiness.json")
else
    arguments=("$@")
fi
printf '%q ' python -u caco_trackA_v1/scripts/summarize_baselines.py "${arguments[@]}" > "$run_dir/command.txt"
printf '\n' >> "$run_dir/command.txt"
...
python -u caco_trackA_v1/scripts/summarize_baselines.py "${arguments[@]}" \
