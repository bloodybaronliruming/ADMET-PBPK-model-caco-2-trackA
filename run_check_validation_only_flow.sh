...
run_id="caco_trackA_validation_flow_$(date -u +%Y%m%dT%H%M%SZ)"
run_dir="progress/runs/$run_id"
mkdir -p "$run_dir"
printf '%s\n' 'python -u caco_trackA_v1/scripts/check_validation_only_flow.py' > "$run_dir/command.txt"
...
python -u caco_trackA_v1/scripts/check_validation_only_flow.py \
